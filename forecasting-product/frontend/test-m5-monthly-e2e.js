/**
 * E2E test — Visit all UI pages with Walmart M5 monthly data and capture screenshots.
 *
 * Prerequisites:
 *   - Backend running on :8000
 *   - Frontend running on :3000
 *   - Monthly M5 demo CSV at ../data/demo/m5_monthly.csv
 *   - npx playwright install chromium
 *
 * Run: node test-m5-monthly-e2e.js
 */
...
const RESULTS_DIR = path.join(__dirname, 'test-results', 'm5-monthly');
const LOB = 'walmart_m5_monthly';
const CSV_PATH = path.resolve(__dirname, '..', 'data', 'demo', 'm5_monthly.csv');
const CONFIG_PATH = path.resolve(__dirname, '..', 'configs', 'm5_monthly_config.yaml');
...
async function main() {
  ...
  if (!fs.existsSync(CSV_PATH)) {
    console.error(`❌ Monthly CSV not found: ${CSV_PATH}`);
    console.error('   Run: python scripts/prepare_m5_monthly.py');
    process.exit(1);
  }
  ...
  // ──────────────── 1. DATA ONBOARDING ────────────────
  console.log('\n1. Data Onboarding — Upload M5 Monthly');
  ...
  console.log('  Uploaded m5_monthly.csv');
  ...
  if (await backtestBtn.isVisible()) {
    ...
    if (!isDisabled) {
      ...
      console.log('  Running backtest (monthly — should be fast)...');
      try {
  ...
  await browser.close();
  console.log(`\n${'='.repeat(50)}`);
  console.log(`M5 Monthly E2E — ${errors === 0 ? '✅ PASS' : `⚠️ ${errors} issue(s)`}`);
